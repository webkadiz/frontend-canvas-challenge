...
test('all 12 operations, eight HEAD/OPTIONS routes and unsupported methods match the contract', async (t) => {
  const h = await harness(t);

  await h.call('PUT', h.space.links.self.href, { title: 'Новое имя' });
  ...
  const paths = {
    ...
    '/api/config': ['GET'],
    '/api/spaces': ['GET', 'POST'],
    [h.space.links.self.href]: ['GET', 'PUT'],
    [h.graphUrl]: ['GET', 'PUT'],
    [h.jobsUrl]: ['GET', 'POST'],
...
test('renaming validates the title and preserves the graph and its ETag', async (t) => {
  const h = await harness(t);
  const url = h.space.links.self.href;
  const renamed = (await h.call('PUT', url, { title: '  Идеи для обложки  ' })).json();

  assert.equal(renamed.title, 'Идеи для обложки');
  assert.equal(renamed.id, h.space.id);
  assert.equal(renamed.createdAt, h.space.createdAt);
  assert.equal((await h.call('GET', url)).json().title, renamed.title);
  assert.equal((await h.call('GET', '/api/spaces')).json()[0].title, renamed.title);

  const graph = await h.call('GET', h.graphUrl);

  assert.equal(graph.body, h.saved.body);
  assert.equal(graph.headers.etag, h.saved.headers.etag);

  for (const title of ['', '   ', 'x'.repeat(81)]) await h.call('PUT', url, { title }, 400);

  await h.call('PUT', '/api/spaces/' + randomUUID(), { title: 'Имя' }, 404);
});
...
test('restart retains renamed space, graph, ETag, jobs and idempotency; reading completion never writes', async (t) => {
  ...
  await h.call('PUT', h.space.links.self.href, { title: 'Сохранённое имя' });
  ...
  assert.equal(
    (await reopened.inject({ method: 'GET', url: h.space.links.self.href })).json().title,
    'Сохранённое имя',
  );
